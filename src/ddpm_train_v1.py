...
from src.dataset.ddpm_dataset_v1 import RobustPrecipitationDataModule
...
class DiffusionLightningModule(pl.LightningModule):
    def __init__(self, learning_rate=1e-4):
        ...
        模型输入为 1 通道条件数据（仅仅是低分辨率降水），输出为 1 通道（降水残差）。
        """
        super().__init__()
        ...
        self.model = Network.EDMPrecond(
            img_resolution=(224, 224),
            in_channels=2,  # 1 通道条件数据 + 1 通道降水残差（作为条件输入）
            out_channels=1,
            label_dim=0,
            use_diffuse=True
        )
...
if __name__ == "__main__":
    ...
    dm = RobustPrecipitationDataModule(train_dir, val_dir, test_dir, batch_size=8,
                                       norm_means=norm_means, norm_stds=norm_stds, residual_mean=residual_mean,
    ...
    save_every_n_epochs_callback = SaveEveryNEpochs(save_dir="../models/unet_diffusion_v1/", save_every_n_epochs=20)
    ...
    trainer = pl.Trainer(
        ...
        default_root_dir="../logs/unet_diffusion_v1/",
    )
    ...
    if best_model_path:
        ...
        torch.save(best_model.model.state_dict(), "../models/unet_diffusion_v1/best_model.pt")
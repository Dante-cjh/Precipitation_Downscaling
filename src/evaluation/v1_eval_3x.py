...
def evaluate_3x(test_dir, model_paths, upscale_factor, output_dir, device):
    ...
    # 数据加载
    test_dataset = PrecipitationDataset(test_dir, (224, 224), (224, 224), upscale_factor=upscale_factor)
    ...
    with torch.no_grad():
        for idx, (lr, hr) in tqdm(enumerate(test_loader), total=len(test_loader), desc="Evaluating"):
            ...
            try:
                ...
                print(f"无法打开测试文件: {os.listdir(test_dir)[idx]}")
                continue
...
if __name__ == "__main__":
    ...
    MODEL_PATHS = [
        "../../models/v1/srcnn_layer1.pth",
        "../../models/v1/srcnn_layer2.pth",
        "../../models/v1/srcnn_layer3.pth"
    ]
    UPSCALE_FACTOR = 2
    OUTPUT_DIR = "../../predictions/v1/"

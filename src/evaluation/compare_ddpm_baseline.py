...
# 读取两个 netCDF 文件
baseline_one_channel_nc_file = "../../predictions/diffusion_unet_v1/results/samples.nc"
baseline_nc_file = "../../predictions/diffusion_unet/results/samples.nc"  # baseline 模型的文件
model_v2_nc_file = "../../predictions/diffusion_unet_v2_80/results/samples.nc"  # 使用交叉注意力模型的文件
model_v3_nc_file = "../../predictions/diffusion_unet_v3/results/samples.nc" # 使用CAM+SAM注意力模型的文件

# 读取数据
baseline_one_channel_ds = xr.open_dataset(baseline_one_channel_nc_file)
baseline_ds = xr.open_dataset(baseline_nc_file)
model_v2_ds = xr.open_dataset(model_v2_nc_file)
model_v3_ds = xr.open_dataset(model_v3_nc_file)

# 获取 predicted 数据
baseline_one_channel = baseline_one_channel_ds["predicted"].values  # [time, lat, lon]
baseline_predicted = baseline_ds["predicted"].values  # [time, lat, lon]
model_v2_predicted = model_v2_ds["predicted"].values  # [time, lat, lon]
model_v3_predicted = model_v3_ds["predicted"].values  # [time, lat, lon]
fine = baseline_ds["fine"].values      # [time, lat, lon]
coarse = baseline_ds["coarse"].values  # [time, lat, lon]

# 确保 predicted 中的负值被置为 0
baseline_one_channel[baseline_one_channel < 0] = 0
baseline_predicted[baseline_predicted < 0] = 0
model_v2_predicted[model_v2_predicted < 0] = 0
model_v3_predicted[model_v3_predicted < 0] = 0
...
for t in tqdm(range(ntime)):
    ...
    baseline_one_channel_img = baseline_one_channel[t]
    baseline_predicted_img = baseline_predicted[t]
    model_v2_predicted_img = model_v2_predicted[t]
    model_v3_predicted_img = model_v3_predicted[t]
    fine_img = fine[t]
    coarse_img = coarse[t]

    # 获取实际数据的最小值和最大值
    data_min = min(coarse_img.min(), fine_img.min())
    data_max = max(coarse_img.max(), fine_img.max())
    ...
    ax.set_title(f"Ground Truth {t + 1}")
    fig.colorbar(c, ax=ax)

    # 显示 baseline one channel 模型的 predicted 图像
    ax = axes[1, 0]
    c = ax.imshow(baseline_one_channel_img, cmap='viridis', vmin=data_min, vmax=data_max)  # 归一化到 [0, 1] 范围
    ax.set_title(f"Baseline One Channel Predicted {t+1}")
    fig.colorbar(c, ax=ax)
    ...
    ax.set_title(f"Model with Self-Attention Predicted {t+1}")
    fig.colorbar(c, ax=ax)

    # 显示 model_v3 模型的 predicted 图像
    ax = axes[2, 1]
    c = ax.imshow(model_v3_predicted_img, cmap='viridis', vmin=data_min, vmax=data_max)
    ax.set_title(f"Model with CAM+SAM Predicted {t+1}")
    fig.colorbar(c, ax=ax)

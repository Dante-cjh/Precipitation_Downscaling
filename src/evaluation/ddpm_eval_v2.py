...
from src.dataset.ddpm_dataset_v2 import RobustPrecipitationZScoreDataset
...
def sample_unet(input_batch, model, device, dataset):
    ...
    # 模型输出预测残差
    residual = model(images_input)  # Since the model in NetworkV2 does not need the sigma as input
...
def sample_diffusion(input_batch, model, device, dataset, num_steps=40,
    ...
    # 合并"conditions"下的"dynamic", "static", "coarse_precip"为images_input
    dynamic = input_batch["conditions"]["dynamic"].to(device)  # [B, 4, H, W]
    static = input_batch["conditions"]["static"].to(device)  # [B, 2, H, W]
    coarse_precip = input_batch["conditions"]["coarse_precip"].to(device)  # [B, 1, H, W]

    # 按通道合并，形成 images_input, 结果形状：[B, 7, H, W]
    images_input = torch.cat([coarse_precip, dynamic, static], dim=1)

    # 从"metadata"中提取fine_precip和coarse_precip作为fine和coarse
    coarse = input_batch["metadata"]["coarse_precip"].to(device)  # [B, 1, H, W]
    fine = input_batch["metadata"]["fine_precip"].to(device)  # [B, 1, H, W]

    meteo_conditions = torch.cat(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        denoised = model(x_hat, t_hat, images_input, meteo_conditions=meteo_conditions).to(torch.float64)
        d_cur = (x_hat - denoised) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur

        if i < num_steps - 1:
            denoised = model(x_next, t_next, images_input, meteo_conditions=meteo_conditions).to(torch.float64)
...
if __name__ == "__main__":
    ...
    model_dir = "../../models/unet_diffusion/"             # 模型保存目录
    save_dir = "../../predictions/diffusion_unet/results/"
    os.makedirs(save_dir, exist_ok=True)

    # 根据模型类型加载模型和采样函数
    if modelname == "Diffusion":
        model = NetworkV2.EDMPrecond(

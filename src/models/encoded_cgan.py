...
class EncodedGenerator(torch.nn.Module):
    ...
    def __init__(self, in_ch, ncvar, cvar_ch=8, relu_a=0.01, use_ele=False, cam=False, sam=False,
                 stage_chs=(256, 128, 64, 32)):
        super().__init__()
        self.in_ch = in_ch
        ...
        self.up3_ops = [
            torch.nn.ConvTranspose2d(in_channels=stage_chs[2], out_channels=stage_chs[2],
                                     kernel_size=2, stride=2, padding=0),
            torch.nn.LeakyReLU(negative_slope=0.01),
        ]
        ...
        self.out_ops = [
            torch.nn.Conv2d(in_channels=stage_chs[3], out_channels=4, \
                            kernel_size=3, stride=1, padding=1),
        ...
        self.up3_layers = torch.nn.Sequential(*self.up3_ops)

        if self.use_cam:
            self.up1_cam = CAM(in_ch=stage_chs[0] + cvar_ch * ncvar)
            self.up2_cam = CAM(in_ch=stage_chs[1])
            self.up3_cam = CAM(in_ch=stage_chs[2])

        if self.use_sam:
            self.up1_sam = SAM(in_ch=stage_chs[0])
            self.up2_sam = SAM(in_ch=stage_chs[1])
            self.up3_sam = SAM(in_ch=stage_chs[2])

        if use_ele:
            self.ele_layers = torch.nn.Sequential(*self.ele_ops)
            self.p4_inception1 = inception_box(in_ch=8 + stage_chs[2], o_ch=stage_chs[3])
        else:
            self.p4_inception1 = inception_box(in_ch=stage_chs[2], o_ch=stage_chs[3])
        self.p4_inception2 = inception_box(in_ch=stage_chs[3], o_ch=stage_chs[3])
        self.p4_inception3 = inception_box(in_ch=stage_chs[3], o_ch=stage_chs[3])
        self.p4_inception4 = inception_box(in_ch=stage_chs[3], o_ch=stage_chs[3])
    ...
    def forward(self, x, cvars, elevation=None, ret_sam=False):
        ...
        if self.use_cam:
            out_tmp = self.up3_cam(out_tmp)  # apply channel attention

        if self.use_sam:  # apply spatial attention
            if ret_sam:
                atten3, out_tmp = self.up3_sam(out_tmp, ret_att=True)
            else:
                out_tmp = self.up3_sam(out_tmp)

        for layer in self.up3_layers:
            out_tmp = layer(out_tmp)
        ...
        out_tmp = self.p4_inception1(out_tmp)
        out_tmp = self.p4_inception2(out_tmp)
        out_tmp = self.p4_inception3(out_tmp)
        out_tmp = self.p4_inception4(out_tmp)
        for layer in self.out_layers:
            out_tmp = layer(out_tmp)

        if ret_sam:
            return out_tmp, atten1, atten2, atten3
        else:
            return out_tmp

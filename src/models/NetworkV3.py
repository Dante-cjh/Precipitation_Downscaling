...
class CAM(torch.nn.Module):
    ...
    def forward(self, x, ret_att=False):
        ...
        _attention = _attention.unsqueeze(-1).unsqueeze(-1)
        if ret_att:
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        # Add CAM and SAM
        if self.use_cam:
            self.cam = CAM(in_ch=out_channels)
        if self.use_sam:
            self.sam = SAM(in_ch=out_channels)
...
class UNet(torch.nn.Module):
    def __init__(self,
        ...
        block_kwargs = dict(emb_channels=emb_channels, channels_per_head=64, dropout=dropout, init=init, init_zero=init_zero, use_cam= use_cam, use_sam = use_sam)
